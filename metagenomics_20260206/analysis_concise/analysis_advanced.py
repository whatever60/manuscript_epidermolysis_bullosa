...
try:
    import analysis_concise_base_kernel as base
...
def load_cluster_results(data_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    host_cluster = pd.read_csv(
        data_dir / "analysis_concise" / "tables" / "table_02_02_host_model.tsv",
        sep="\t",
    )
...
def main() -> None:
    ...
    context = AdvancedContext(
        data_dir=data_dir,
        output_dir=data_dir / "analysis_concise" / "advanced",
        figure_dir=data_dir / "analysis_concise" / "advanced" / "figures",
        table_dir=data_dir / "analysis_concise" / "advanced" / "tables",
        input_dir=data_dir / "analysis_concise" / "advanced" / "inputs",
    )
    ensure_dirs(context)
