...
def fit_host_model(qc: pd.DataFrame) -> tuple[object, pd.DataFrame]:
    model_df = qc.dropna(subset=["host_logit", "body_region", "patient_id", "culture_date"]).copy()
    model_df["collection_year"] = model_df["culture_date"].dt.year.astype(int)
    formula = "host_logit ~ C(body_region, Treatment('lower_extremity')) + C(collection_year)"
...
def make_qc_figure(qc: pd.DataFrame, context: AnalysisContext) -> None:
    ...
    fig.tight_layout()
    fig.savefig(context.figure_dir / "figure_01_qc_host_burden.svg", bbox_inches="tight")
    plt.close(fig)
...
def summarize_pairwise_distances(rel_abundance: pd.DataFrame, metadata: pd.DataFrame) -> pd.DataFrame:
    ...
    for i, sample_i in enumerate(samples):
        for j in range(i + 1, len(samples)):
            ...
            same_visit = meta_i["visit_id"] == meta_j["visit_id"]
            if same_patient and same_visit:
                group = "Same patient, same visit"
            elif same_patient:
                group = "Same patient, different visit"
            else:
...
def make_distance_figure(pairwise: pd.DataFrame, context: AnalysisContext) -> pd.DataFrame:
    ...
    ax.set_title("Within-patient samples are more similar than unrelated samples")
    ...
    fig.tight_layout()
    fig.savefig(context.figure_dir / "figure_02_pairwise_distance.svg", bbox_inches="tight")
    plt.close(fig)
...
def make_culture_figure(summary: pd.DataFrame, plot_df: pd.DataFrame, context: AnalysisContext) -> None:
    ...
    for ax, label in zip(axes, top_labels):
        ...
        ax.set_title(f"{label}\nAUROC={metrics['auroc']:.2f}, q={metrics['qvalue']:.3g}")
        ax.set_xlabel("")
    ...
    fig.tight_layout()
    fig.savefig(context.figure_dir / "figure_03_culture_concordance.svg", bbox_inches="tight")
    plt.close(fig)
...
def make_species_association_figure(results: pd.DataFrame, context: AnalysisContext) -> pd.DataFrame:
    ...
    fig.tight_layout()
    fig.savefig(context.figure_dir / "figure_04_species_associations.svg", bbox_inches="tight")
    plt.close(fig)
...
def write_report(
    ...
    host_year = host_results.loc[host_results["term"].str.contains("collection_year")].sort_values("pvalue").head(1)
    pair_top = pairwise_summary.loc[pairwise_summary["comparison_group"] == "Same patient, same visit"].head(1)
    pair_revisit = pairwise_summary.loc[pairwise_summary["comparison_group"] == "Same patient, different visit"].head(1)
    ...
    if not host_region.empty:
        ...
        lines.append(
            f"- Upper-extremity swabs trended toward higher host burden than lower-extremity swabs in the depth-adjusted model (coefficient {row['estimate']:.2f} on the logit host scale, q={row['qvalue']:.3g}), but collection year was an even stronger driver."
        )
    if not host_year.empty:
        row = host_year.iloc[0]
        lines.append(
            f"- Host burden also shifted strongly over time; the largest collection-year term was `{row['term'].split('[')[-1].rstrip(']')}` with coefficient {row['estimate']:.2f} (q={row['qvalue']:.3g})."
        )

    if not pair_top.empty:
        row = pair_top.iloc[0]
        lines.append(
            f"- Same-patient, same-visit pairs had median Bray-Curtis distance {row['median']:.3f}, compared with {pairwise_summary.loc[pairwise_summary['comparison_group'] == 'Different patient', 'median'].iloc[0]:.3f} for unrelated pairs."
        )
    if not pair_revisit.empty:
        row = pair_revisit.iloc[0]
        lines.append(
            f"- Same-patient, different-visit pairs were much less stable: median distance was {row['median']:.3f}, essentially similar to unrelated pairs."
        )
...
def main() -> None:
    ...
    context = AnalysisContext(
        ...
        figure_dir=data_dir / "analysis" / "figures",
        table_dir=data_dir / "analysis" / "tables",
    )
