...
import run_analysis as base


HOST_FORMULAS = {
    "host_base": "host_logit ~ C(body_region, Treatment('lower_extremity')) + C(collection_year)",
    "host_extended": (
        "host_logit ~ C(body_region, Treatment('lower_extremity')) "
        "+ C(chronicity_group, Treatment('unknown')) + C(collection_year)"
    ),
}
...
def fit_variance_component_model(
    ...
    frame = data.copy()
    frame["all_group"] = "all"
    vc_formula = {"patient": "0 + C(patient_id)", "visit": "0 + C(visit_id)"}
    ...
    status = {
        ...
        "visit_var": float(fit.params.get("visit Var", np.nan)),
        "optimizer": optimizer,
...
def fit_host_models(qc: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    model_df = qc.dropna(
        subset=["host_logit", "body_region", "patient_id", "visit_id", "collection_year", "chronicity_group"]
    ).copy()
    effect_tables = []
    status_rows = []
...
def fit_species_models(qc: pd.DataFrame, species_bac: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    metadata = qc.loc[
        sample_ids,
        ["patient_id", "visit_id", "body_region", "chronicity_group", "log10_bacterial_reads"],
    ].copy()
...
def load_cluster_results(data_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    host_cluster = pd.read_csv(data_dir / "analysis" / "tables" / "host_model.tsv", sep="\t")
    host_cluster["outcome"] = "host_logit"
    host_cluster["model_name"] = "cluster_ols"

    species_cluster = pd.read_csv(data_dir / "analysis" / "tables" / "species_associations.tsv", sep="\t")
...
def write_report(
    ...
    mixed_status = species_status.loc[:, ["outcome", "aic", "patient_var", "visit_var", "warning_count"]]
    ...
    lines.append(
        f"- Host base model variance components: patient {host_status_row['patient_var']:.3g}, visit {host_status_row['visit_var']:.3g}."
    )
...
def main() -> None:
    ...
    context = AdvancedContext(
        data_dir=data_dir,
        output_dir=data_dir / "analysis" / "advanced",
        figure_dir=data_dir / "analysis" / "advanced" / "figures",
        table_dir=data_dir / "analysis" / "advanced" / "tables",
        input_dir=data_dir / "analysis" / "advanced" / "inputs",
    )
    ensure_dirs(context)
